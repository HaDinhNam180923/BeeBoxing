...
const ManageProducts = ({ collectionId }) => {
  ...
  const fetchCollectionProducts = async () => {
    try {
      const response = await axios.get(`/api/admin/collections/${collectionId}`);
      ...
      if (response.data.status) {
        ...
        setSelectedProducts(collectionProducts.map(product => ({
          ...
          name: product.name,
          image: product.colors?.[0]?.images?.[0]?.image_url || '/storage/images/product-placeholder.jpg',
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/admin/collections/products/selection', {
        params: {
          search,
          page
        }
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(`/api/admin/collections/${collectionId}/products`, {
  ...
  return (
    <AdminLayout title={`Quản lý sản phẩm - ${collectionId}`}>
      <AdminSidebar>
        <div className="mt-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold">Quản lý sản phẩm trong bộ sưu tập</h1>
            <Link
